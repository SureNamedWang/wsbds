    ...
    <script type="text/javascript">
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            while ($row = mysqli_fetch_assoc($result)) { ?>
                ...
                var wkt<?php echo $row["id"];?> = 'POINT(<?php echo $row["y"];?> <?php echo $row["x"];?>)';

                var format<?php echo $row["id"];?> = new ol.format.WKT();

                var feature<?php echo $row["id"];?> = format<?php echo $row["id"];?>.readFeature(wkt<?php echo $row["id"];?>, {
                    dataProjection: 'EPSG:4326',
                    featureProjection: 'EPSG:3857'
                });

                var vector<?php echo $row["id"];?> = new ol.layer.Vector({
                    source: new ol.source.Vector({
                        features: [feature<?php echo $row["id"];?>]
                    })
                }); 
